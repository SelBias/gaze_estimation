...
def hetero_precision_without_val(
    ...
    mean_network, hidden_features, K=2, init_log_lamb=0, initialize_Sigma=True,
    ...
    precision   = precision_module(p, K, init_log_lamb, device=device, dtype=dtype).to(device)
    ...
    precision_list = np.zeros((3, K, p, p))
    ...
    for iter in tqdm(range(pretrain_iter)) : 
        ...
        for epoch in range(m_pretrain_epoch) : 
            for _, (image, hp, y, batch_ids) in enumerate(train_loader) : 
                ...
                pretrain_loss = nhll_hetero_precision(train_N, y, batch_fixed, batch_random, v_list, weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, init_log_lamb=init_log_lamb, update='pretrain', verbose=verbose)
        ...
        with torch.no_grad() : 
            ...
            # Sigma_v = Sigma.recover_precision()
            ...
            train_nhll = nhll_hetero_precision(train_N, train_y_cuda, train_fixed, train_random, v_list, Gamma_list=train_Gamma_list, init_log_lamb=init_log_lamb, update='pretrain-eval', verbose=verbose).item()
            # train_nll  = sum([multivariate_nll_hetero_i(train_y_list[i], train_fixed_list[i], train_Gamma_list[i], 0, torch.ones(train_n_list[i],K, device=device), Sigma_v) for i in range(m)]).item() / train_N
            # train_njll = sum([multivariate_njll_hetero_i(train_y_list[i], train_fixed_list[i], train_Gamma_list[i], v_list[i], torch.ones(train_n_list[i],K, device=device), Sigma_v) for i in range(m)]).item() / train_N
            # train_nhll = sum([multivariate_nhll_hetero_i(train_y_list[i], train_fixed_list[i], train_Gamma_list[i], v_list[i], torch.ones(train_n_list[i],K, device=device), Sigma_v) for i in range(m)]).item() / train_N
            # train_nhll = nhll_hetero_arbitrary(train_N, train_y_cuda, train_fixed, train_random, v_list, torch.zeros(train_N,K, device=device), Sigma=None, 
            #                                    Gamma_list = train_Gamma_list, log_phi_list = [torch.zeros(train_n_list[i],K, device=device) for i in range(m)], update='V', verbose=verbose).item()
            ...
            if test_unseen is True : 
                ...
                # test_nll  = sum([multivariate_nll_hetero_i(test_y_list[i], test_fixed_list[i], test_Gamma_list[i], 0, torch.ones(test_n_list[i],K, device=device), Sigma_v) for i in range(test_m)]).item() / test_N
                ...
                # test_nll_adjusted  = sum([multivariate_nll_hetero_i(test_y_list[i], test_fixed_list[i] + test_adjusted_list[i], test_Gamma_list[i], 0, torch.ones(test_n_list[i],K, device=device), Sigma_v) for i in range(test_m)]).item() / test_N
                ...
                # test_nll  = sum([multivariate_nll_hetero_i(test_y_list[i], test_fixed_list[i], test_Gamma_list[i], 0, torch.ones(test_n_list[i],K, device=device), Sigma_v) for i in range(m)]).item() / test_N
                # test_njll = sum([multivariate_njll_hetero_i(test_y_list[i], test_fixed_list[i], test_Gamma_list[i], v_list[i], torch.ones(test_n_list[i],K, device=device), Sigma_v) for i in range(m)]).item() / test_N
                test_nhll = nhll_hetero_precision(test_N, test_y_cuda, test_fixed, test_random, v_list, Gamma_list=test_Gamma_list, init_log_lamb=init_log_lamb, update='pretrain-eval', verbose=verbose).item()
                
                # test_nhll = sum([multivariate_nhll_hetero_i(test_y_list[i], test_fixed_list[i], test_Gamma_list[i], v_list[i], torch.ones(test_n_list[i],K, device=device), Sigma_v) for i in range(m)]).item() / test_N
                # test_nhll = nhll_hetero_arbitrary(test_N, test_y_cuda, test_fixed, test_random, v_list, torch.zeros(test_N ,K, device=device), Sigma=None, Gamma_list = test_Gamma_list,
                #                                   log_phi_list = [torch.zeros(test_n_list[i], K, device=device) for i in range(test_m)], update='V', verbose=verbose).item()
    ...
    torch.save(mean_model.state_dict(), f'./results/hetero_prec_{experiment_name}_pretrained_mean_model.pt')
    torch.save(log_phi_layer.state_dict(), f'./results/hetero_prec_{experiment_name}_pretrained_variance_model.pt')
    torch.save(precision.state_dict(), f'./results/hetero_prec_{experiment_name}_pretrained_precision.pt')
    ...
    for iter in tqdm(range(max_iter)) : 
        ...
        for epoch in range(mean_epoch) : 
            ...
            for _, (image, hp, y, batch_ids) in enumerate(train_loader) : 
                ...
                mean_optimizer.zero_grad()
                variance_optimizer.zero_grad()
                train_loss = nhll_hetero_precision(train_N, y, batch_fixed, batch_random, v_list, batch_log_phi, weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, LT=LT, update='M', verbose=verbose)
                # train_loss = nhll_hetero_arbitrary(train_N, y, batch_fixed, batch_random, v_list, batch_log_phi, Sigma, weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, L_inv=L_inv, update='M', verbose=verbose)
                train_loss.backward()
                mean_optimizer.step()
            ...
            with torch.no_grad() : 
                ...
                mean_optimizer.zero_grad()
                variance_optimizer.zero_grad()
                train_loss = nhll_hetero_precision(train_N, train_y_cuda, train_fixed, train_random, v_list, train_log_phi, precision, 
                                                   train_Gamma_list, train_log_phi_list, update='V', verbose=verbose)
                # train_loss = nhll_hetero_arbitrary(train_N, train_y_cuda, train_fixed, train_random, v_list, train_log_phi, Sigma, 
        ...
        with torch.no_grad() : 
            ...
            train_mae = mae(train_y_cuda, train_fixed + train_random, is_3d=False, deg=deg).item()
            train_mse = F.mse_loss(train_y_cuda, train_fixed + train_random).item()
            train_nhll = nhll_hetero_precision(train_N, train_y_cuda, train_fixed, train_random, v_list, train_log_phi, precision, 
                                                train_Gamma_list, train_log_phi_list, update='V', verbose=verbose).item()
            # train_nll = sum([multivariate_nll_hetero_i(train_y_list[i], train_fixed_list[i], train_Gamma_list[i], 0, torch.exp(train_log_phi_list[i]), Sigma_v) for i in range(m)]).item() / train_N
            # train_njll = sum([multivariate_njll_hetero_i(train_y_list[i], train_fixed_list[i], train_Gamma_list[i], v_list[i], torch.exp(train_log_phi_list[i]), Sigma_v) for i in range(m)]).item() / train_N
            # train_nhll = nhll_hetero_arbitrary(train_N, train_y_cuda, train_fixed, train_random, v_list, train_log_phi, Sigma, 
            ...
            if test_unseen is True : 
                test_mae = mae(test_y_cuda, test_fixed, is_3d=False, deg=deg).item()
                test_mse = F.mse_loss(test_y_cuda, test_fixed).item()
                # test_nll = sum([multivariate_nll_hetero_i(test_y_list[i], test_fixed_list[i], test_Gamma_list[i], 0, torch.exp(test_log_phi_list[i]), Sigma_v) for i in range(test_m)]).item() / test_N
                
                test_mae_adjusted = mae(test_y_cuda, test_fixed + test_adjusted, is_3d=False, deg=deg).item()
                test_mse_adjusted = F.mse_loss(test_y_cuda, test_fixed + test_adjusted).item()
                # test_nll_adjusted = sum([multivariate_nll_hetero_i(test_y_list[i], test_fixed_list[i] + test_adjusted_list[i], test_Gamma_list[i], 0, torch.exp(test_log_phi_list[i]), Sigma_v) for i in range(test_m)]).item() / test_N
                ...
                test_mae = mae(test_y_cuda, test_fixed + test_random, is_3d=False, deg=deg).item()
                test_mse = F.mse_loss(test_y_cuda, test_fixed + test_random).item()
                test_nhll = nhll_hetero_precision(test_N, test_y_cuda, test_fixed, test_random, v_list, test_log_phi, precision, 
                                                  test_Gamma_list, test_log_phi_list, update='V', verbose=verbose).item()
                # test_nll = sum([multivariate_nll_hetero_i(test_y_list[i], test_fixed_list[i], test_Gamma_list[i], 0, torch.exp(test_log_phi_list[i]), Sigma_v) for i in range(m)]).item() / test_N
                # test_njll = sum([multivariate_njll_hetero_i(test_y_list[i], test_fixed_list[i], test_Gamma_list[i], v_list[i], torch.exp(test_log_phi_list[i]), Sigma_v) for i in range(m)]).item() / test_N
                # # test_nhll = sum([multivariate_nhll_hetero_i(test_y_list[i], test_fixed_list[i], test_Gamma_list[i], v_list[i], torch.exp(test_log_phi_list[i]), Sigma_v) for i in range(m)]).item() / test_N
                # test_nhll = nhll_hetero_arbitrary(test_N, test_y_cuda, test_fixed, test_random, v_list, test_log_phi, Sigma, test_Gamma_list, test_log_phi_list, update='V', verbose=verbose).item()
                ...
                print(f'{iter}-th main train test MAE, MSE, NHLL : {test_mae:.4f} deg, {test_mse:.4f}, {test_nhll:.4f}')
    ...
    np.save(f'./prediction/hetero_prec_{experiment_name}_pred', prediction)
    np.save(f'./prediction/hetero_prec_{experiment_name}_pred_adjusted', prediction_adjusted)
    np.save(f'./results/hetero_prec_{experiment_name}_train_loss', train_loss_list)
    np.save(f'./results/hetero_prec_{experiment_name}_test_loss', test_loss_list)
    np.save(f'./results/hetero_prec_{experiment_name}_v_list', v_list_list)
    np.save(f'./results/hetero_prec_{experiment_name}_precision', precision_list)
    np.save(f'./results/hetero_prec_{experiment_name}_beta', beta_list)
    np.save(f'./results/hetero_prec_{experiment_name}_w', w_list)
    np.save(f'./results/hetero_prec_{experiment_name}_train_Gamma', train_Gamma_list_list)
    np.save(f'./results/hetero_prec_{experiment_name}_test_Gamma', test_Gamma_list_list)

    torch.save(mean_model.state_dict(), f'./results/hetero_prec_{experiment_name}_trained_mean_model.pt')
    torch.save(best_nhll_mean_model.state_dict(), f'./results/hetero_prec_{experiment_name}_nhll_selected_mean_model.pt')
    
    torch.save(log_phi_layer.state_dict(), f'./results/hetero_prec_{experiment_name}_trained_variance_model.pt')
    torch.save(best_nhll_log_phi_layer.state_dict(), f'./results/hetero_prec_{experiment_name}_nhll_selected_variance_model.pt')
    
    torch.save(precision.state_dict(), f'./results/hetero_prec_{experiment_name}_trained_precision.pt')
    torch.save(best_nhll_precision.state_dict(), f'./results/hetero_prec_{experiment_name}_nhll_selected_precision.pt')
