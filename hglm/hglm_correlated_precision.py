...
def correlated_precision_without_val(
    ...
    precision = large_precision_module(p, K, device=device).to(device)
    ...
    for iter in tqdm(range(pretrain_iter)) : 
        ...
        for epoch in range(m_pretrain_epoch) : 
            for _, (image, hp, y, batch_ids) in enumerate(train_loader) : 
                ...
                pretrain_loss = nhll_correlated_precision(
                    train_N, y, batch_fixed, batch_random, v_list, 
                    weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, update='pretrain', verbose=verbose)
        ...
        with torch.no_grad() : 
            ...
            train_nhll = nhll_correlated_precision(
                train_N, train_y_cuda, train_fixed, train_random, v_list, 
                Gamma_list = train_Gamma_list, update='pretrain-eval', verbose=verbose).item()

            train_loss_list[0, iter] = train_mae
            train_loss_list[1, iter] = train_mse
            train_loss_list[4, iter] = train_nhll
            print(f'{iter}-th Pretrain train MAE, MSE, NHL : {train_mae:.4f} deg, {train_mse:.4f}, {train_nhll:.4f}')
            ...
            if test_unseen is True : 
                ...
                test_nhll = nhll_correlated_precision(
                    test_N, test_y_cuda, test_fixed, test_random, v_list, 
                    Gamma_list = test_Gamma_list, update='pretrain-eval', verbose=verbose).item()
                ...
                print(f'{iter}-th Pretrain test MAE, MSE, NHL : {test_mae:.4f} deg, {test_mse:.4f}, {test_nhll:.4f}')
    ...
    for iter in tqdm(range(max_iter)) : 
        ...
        for epoch in range(mean_epoch) : 
            ...
            for _, (image, hp, y, batch_ids) in enumerate(train_loader) : 
                ...
                mean_optimizer.zero_grad()
                variance_optimizer.zero_grad()
                train_loss = nhll_correlated_precision(train_N, y, batch_fixed, batch_random, v_list, batch_log_phi, weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, LT=LT, update='M', verbose=verbose)
                train_loss.backward()
                mean_optimizer.step()
            ...
            with torch.no_grad() : 
                ...
                mean_optimizer.zero_grad()
                variance_optimizer.zero_grad()
                train_loss = nhll_correlated_precision(
                    train_N, train_y_cuda, train_fixed, train_random, v_list, train_log_phi, precision, 
        ...
        with torch.no_grad() : 
            ...
            if test_unseen is True : 
                ...
                test_mse = F.mse_loss(test_y_cuda, test_fixed + test_random).item()
                test_nhll = nhll_correlated_precision(
                    test_N, test_y_cuda, test_fixed, test_random, v_list, test_log_phi, precision, 
    ...
    if test_unseen is True : 
        ...
        print(f'NHLL-selected model train MAE, MSE, NHL : {train_loss_list[0,best_nhll_index]:.4f} deg, {train_loss_list[1,best_nhll_index]:.4f}, {train_loss_list[2,best_nhll_index]:.4f}, {train_loss_list[3,best_nhll_index]:.4f}, {train_loss_list[4,best_nhll_index]:.4f}')
    ...
    np.save(f'./prediction/corr_prec_{experiment_name}_pred', prediction)
    np.save(f'./prediction/corr_prec_{experiment_name}_train_loss', train_loss_list)
    np.save(f'./prediction/corr_prec_{experiment_name}_test_loss', test_loss_list)
    np.save(f'./prediction/corr_prec_{experiment_name}_v_list', v_list_list)
    np.save(f'./prediction/corr_prec_{experiment_name}_precision', precision_list)
    np.save(f'./prediction/corr_prec_{experiment_name}_beta', beta_list)
