...
def correlated_arbitrary_without_val(
    ...
    for iter in tqdm(range(pretrain_iter)) : 
        ...
        for epoch in range(m_pretrain_epoch) : 
            for _, (image, hp, y, batch_ids) in enumerate(train_loader) : 
                ...
                pretrain_loss = nhll_correlated_arbitrary(train_N, y, batch_fixed, batch_random, v_list, weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, update='pretrain', verbose=verbose)
        ...
        with torch.no_grad() : 
            ...
            train_nhll = nhll_correlated_arbitrary(train_N, train_y_cuda, train_fixed, train_random, v_list, Gamma_list = train_Gamma_list, update='pretrain-eval', verbose=verbose)
            ...
            if test_unseen is True : 
                ...
                test_nhll = nhll_correlated_arbitrary(test_N, test_y_cuda, test_fixed, test_random, v_list,Gamma_list = test_Gamma_list, update='pretrain-eval', verbose=verbose).item()
    ...
    torch.save(Sigma.state_dict(), f'./Model/hetero_arbit_{experiment_name}_pretrained_Sigma.pt')
    ...
    for iter in tqdm(range(max_iter)) : 
        ...
        for epoch in range(mean_epoch) : 
            ...
            for _, (image, hp, y, batch_ids) in enumerate(train_loader) : 
                ...
                mean_optimizer.zero_grad()
                variance_optimizer.zero_grad()
                train_loss = nhll_correlated_arbitrary(train_N, y, batch_fixed, batch_random, v_list, batch_log_phi, Sigma, weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, L_inv=L_inv, update='M', verbose=verbose)
                train_loss.backward()
                mean_optimizer.step()
            ...
            with torch.no_grad() : 
                ...
                mean_optimizer.zero_grad()
                variance_optimizer.zero_grad()
                train_loss = nhll_correlated_arbitrary(train_N, train_y_cuda, train_fixed, train_random, v_list, train_log_phi, Sigma, 
        ...
        with torch.no_grad() : 
            ...
            train_mae = mae(train_y_cuda, train_fixed + train_random, is_3d=False, deg=deg).item()
            train_mse = F.mse_loss(train_y_cuda, train_fixed + train_random).item()
            train_nhll = nhll_correlated_arbitrary(train_N, train_y_cuda, train_fixed, train_random, v_list, train_log_phi, Sigma, 
            ...
            if test_unseen is True : 
                ...
                test_mae = mae(test_y_cuda, test_fixed + test_random, is_3d=False, deg=deg).item()
                test_mse = F.mse_loss(test_y_cuda, test_fixed + test_random).item()
                test_nhll = nhll_correlated_arbitrary(test_N, test_y_cuda, test_fixed, test_random, v_list, test_log_phi, Sigma, test_Gamma_list, test_log_phi_list, update='V', verbose=verbose).item()
    ...
    np.save(f'./Prediction/correlated_arbit_{experiment_name}_Sigma', Sigma_list)
    ...
    torch.save(Sigma.state_dict(), f'./Model/correlated_arbit{experiment_name}_trained_Sigma.pt')
    torch.save(best_nhll_Sigma.state_dict(), f'./Model/correlated_arbit_{experiment_name}_nhll_selected_Sigma.pt')
