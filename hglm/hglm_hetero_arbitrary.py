...
def hetero_covariance_without_val(
    ...
    variance_optimizer = optim.Adam(list(log_phi_layer.parameters()) + list(Sigma.parameters()), lr=variance_lr, weight_decay=weight_decay)
    ...
    for iter in tqdm(range(pretrain_iter)) : 
        ...
        for epoch in range(m_pretrain_epoch) : 
            for _, (image, hp, y, batch_ids) in enumerate(train_loader) : 
                ...
                pretrain_loss = nhll_hetero_arbitrary(train_N, y, batch_fixed, batch_random, v_list, weighted=weighted, n_list=train_n_list, batch_n_list=batch_n_list, update='pretrain', verbose=verbose)
    ...
    torch.save(mean_model.state_dict(), f'./Model/hetero_cov{experiment_name}_pretrained_mean_model.pt')
    torch.save(log_phi_layer.state_dict(), f'./Model/hetero_cov{experiment_name}_pretrained_variance_model.pt')
    torch.save(Sigma.state_dict(), f'./Model/hetero_cov{experiment_name}_pretrained_Sigma.pt')
    ...
    for iter in tqdm(range(max_iter)) : 
        ...
        with torch.no_grad() : 
            ...
            if test_unseen is True : 
                ...
                print(f'{iter}-th main train test MAE, MSE, NLL : {test_mae_adjusted:.4f} deg, {test_mse_adjusted:.4f}, {test_nll_adjusted:.4f}')
    ...
    np.save(f'./Prediction/hetero_cov{experiment_name}_train_loss', train_loss_list)
    np.save(f'./Prediction/hetero_cov{experiment_name}_test_loss', test_loss_list)
    np.save(f'./Prediction/hetero_cov{experiment_name}_v_list', v_list_list)
    np.save(f'./Prediction/hetero_cov{experiment_name}_Sigma', Sigma_list)
    np.save(f'./Prediction/hetero_cov{experiment_name}_beta', beta_list)
    np.save(f'./Prediction/hetero_cov{experiment_name}_w', w_list)
    np.save(f'./Prediction/hetero_cov{experiment_name}_train_Gamma', train_Gamma_list_list)
    np.save(f'./Prediction/hetero_cov{experiment_name}_test_Gamma', test_Gamma_list_list)

    torch.save(mean_model.state_dict(), f'./Model/hetero_cov{experiment_name}_trained_mean_model.pt')
    torch.save(best_nhll_mean_model.state_dict(), f'./Model/hetero_cov{experiment_name}_nhll_selected_mean_model.pt')
    
    torch.save(log_phi_layer.state_dict(), f'./Model/hetero_cov{experiment_name}_trained_variance_model.pt')
    torch.save(best_nhll_log_phi_layer.state_dict(), f'./Model/hetero_cov{experiment_name}_nhll_selected_variance_model.pt')
    
    torch.save(Sigma.state_dict(), f'./Model/hetero_cov{experiment_name}_trained_Sigma.pt')
    torch.save(best_nhll_Sigma.state_dict(), f'./Model/hetero_cov{experiment_name}_nhll_selected_Sigma.pt')
